...
export const productStore = reactive({
  ...
  async fetchAll(force = false) {
    ...
    try {
      ...
      // Mutate in place so existing references stay reactive.
  ...
  getById(id) {
    return this.items.find((p) => p.id === Number(id)) || null
  },

  get categories() {
    return [...new Set(this.items.map((p) => p.category).filter(Boolean))]
  },
})
...
export const userStore = reactive({
  ...
  async login(userData, tokenData) {
    ...
    // Merge any items the guest added before logging in, then load server cart.
  ...
  // Validate the stored token against the backend on app start.
  async init() {
    ...
    try {
      ...
      // Token expired/invalid — clear the session silently.
      this.logout()
    }
  },
})
...
export const cartStore = reactive({
  ...
  // ── Persist / fetch ──────────────────────────────────────
  _persistGuest() {
    if (!this.isServer) saveGuestCart(this.items)
  },

  // Map a server cart row into the UI item shape, enriching with
  // catalog details (category, original price, specs, badge image).
  _mapServerItem(row) {
  ...
  async updateQuantity(productId, quantity) {
    ...
    if (this.isServer) {
      ...
      try {
        if (delta > 0) {
          for (let n = 0; n < delta; n++) await cartApi.increase(item.cartId)
        } else if (delta < 0) {
          for (let n = 0; n < -delta; n++) await cartApi.decrease(item.cartId)
        }
      } catch (err) {
  ...
  async clearCart() {
    if (this.isServer) {
      try {
        ...
        // A 404 ("already empty") is not a real error for the UI.
        if (err.status !== 404) this.error = err.message
      }
      await this.fetch()
      return
    }
  ...
  // Drop in-memory items without touching the server (used on logout).
  reset() {
    this.items.splice(0, this.items.length, ...loadGuestCart())
  },

  // On login, push the guest cart to the server, then load the merged cart.
      ...
      for (const item of guestItems) {
        try {
          ...
          for (let n = 1; n < item.quantity; n++) {
            // increase needs the cart row id; refetch to obtain it.
            const data = await cartApi.get()
            const row = (data.items || []).find((r) => r.product_id === item.id)
            if (row) await cartApi.increase(row.cart_id)
          }
        } catch {
