  ...
  <main class="pt-24 pb-16 min-h-screen" style="background: #f9fafb;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            @click="selectedBadge = ''"
            class="px-3 py-1 rounded-full text-xs font-medium transition-all"
            :class="selectedBadge === '' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-100'"
            style="border: 1px solid #e5e7eb;"
          >
            Semua Tag
          </button>
          ...
          <button
            ...
            class="px-3 py-1 rounded-full text-xs font-medium transition-all"
            :class="selectedBadge.toLowerCase().trim() === badge.toLowerCase().trim() ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-100'"
            style="border: 1px solid #e5e7eb;"
          >
            {{ badge }}
          </button>
      ...
      <div v-else class="text-center py-20">
        ...
        <h3 class="text-gray-900 text-xl font-bold mb-2">Produk Tidak Ditemukan</h3>
...
const badges = [...new Set(products.map(p => p.badge.trim()))]
...
const filteredProducts = computed(() => {
  ...
  if (selectedBadge.value) {
    result = result.filter(p =>
      p.badge.toLowerCase().trim() === selectedBadge.value.toLowerCase().trim()
    )
  }
