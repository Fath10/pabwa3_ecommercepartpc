  ...
  <div class="min-h-screen font-sans" style="background: #0d1117;">
    ...
    <Teleport to="body">
      ...
      <Transition name="chat-panel">
        <div
          v-if="chatOpen"
          class="fixed bottom-28 right-6 z-[9998] w-80 max-w-[calc(100vw-3rem)] rounded-2xl overflow-hidden shadow-2xl flex flex-col"
          style="background: #161b22; border: 1px solid rgba(255,255,255,0.1); max-height: 70vh;"
        >
          <!-- Header panel -->
          <div class="flex items-center justify-between px-4 py-3"
            ...
            <button @click="chatOpen = false"
              class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all">
              ×
            </button>
          </div>

          <!-- Area pesan -->
          <div ref="chatBody" class="flex-1 overflow-y-auto p-4 space-y-3" style="min-height: 160px;">
            <div v-for="(msg, i) in chatMessages" :key="i" class="flex" :class="msg.role === 'user' ? 'justify-end' : 'justify-start'">
              <div
                class="px-3.5 py-2 rounded-2xl text-sm max-w-[80%] leading-relaxed"
                :class="msg.role === 'user'
            ...
            <div v-if="chatLoading" class="text-xs text-gray-500 italic">Asisten sedang mengetik…</div>
          </div>

          <!-- Input -->
          <div class="p-3 flex gap-2" style="border-top: 1px solid rgba(255,255,255,0.08);">
            <input
              v-model="chatInput"
              @keyup.enter="sendChatMessage"
              type="text"
              placeholder="Tanya soal rakitan PC…"
              class="flex-1 px-3 py-2 rounded-lg outline-none text-sm text-white"
              style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1);"
            />
            <button @click="sendChatMessage"
              class="px-3 py-2 rounded-lg text-white text-sm font-semibold transition-all hover:opacity-90"
      ...
      >
        <span class="transition-transform duration-300" :class="chatOpen ? 'rotate-90' : ''">
          {{ chatOpen ? '✕' : '🤖' }}
        </span>
      </button>
    </Teleport>
